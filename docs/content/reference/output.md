...
describes what users get. Pick a format with `-o`, or let defillama choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
defillama <command> -o table   # aligned columns for reading
defillama <command> -o jsonl   # one JSON object per line, for piping
defillama <command> -o json    # a single JSON array
defillama <command> -o csv     # spreadsheet friendly
defillama <command> -o tsv     # tab-separated
defillama <command> -o url     # just the URL column
defillama <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
defillama <command> --fields id,title,url
```
...
```bash
defillama <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
defillama <command>            # a table, because this is a terminal
defillama <command> | wc -l    # JSONL, because this is a pipe
```
